...
- 如果能获取 `API calls in the last 30 days` 或 Build 页面中的 `last_month_api_invocation_count`，默认使用混合 TopN 选择基础池前 20 个免费模型，可用 `--top-free-models` 调整基础池数量；`--priority-models` 匹配的重要中文模型会在基础池之外额外加入。
