...
3. 新模型保底池：从最近上架的免费模型里保底选择 1–2 个，避免刚爆火的新模型被老模型挤出。
4. 补位池：如果某个池数量不足，用 30 天 API 调用量靠前的模型补齐。

默认 Top20 配比大致是：稳定热门 14 个、新晋热门 4 个、新模型保底 2 个。这样既保留高请求数量的老模型，也能覆盖类似 `z-ai/glm-5.2` 这种刚上架但增长很快的新模型，同时不增加测试请求总量。
...
```powershell
nvidia-probe run --top-free-models 20 --stable-top-ratio 0.65 --trending-models 5 --newest-models 2 --new-model-days 14
```
...
已使用混合 TopN 策略选择候选模型：稳定热门=14，新晋热门=3，新模型保底=0，补位=3。
...
- 如果检测过程中按 Ctrl+C 中断，工具会保存当前状态并跳过卸载提示，避免在异常退出时删除正在运行的程序目录并产生 traceback。
...
| 默认测试数量 | 混合 TopN 取免费模型前 20 个，约 14 个稳定热门 + 4 个新晋热门 + 2 个新模型保底 |
