...
在 Linux 远程服务器上，可以直接执行一条命令完成拉取、安装、运行。命令启动后会第一时间读取 API Key：如果已经设置 `NVIDIA_API_KEY` 或 `NGC_API_KEY`，会直接使用；否则会立即提示隐藏输入 API Key。输入完成后，脚本才会继续检查 `git`、`python3` 和 Python `venv/ensurepip`，并自动拉取、安装和开始测试；用户无需继续盯着终端，直到检测结束后再处理最终的保留/卸载提示。

推荐使用下面这种 `bash -c "$(curl ...)"` 形式，而不是 `curl ... | bash`，这样脚本可以从交互式终端安全读取 API Key：
...
输入 API Key 后，安装、更新、模型筛选和检测过程会自动完成。默认正常运行结束后才会询问是否卸载程序。一键运行时，测试结果默认保存到当前目录的 `nvidia_probe_results`，程序安装目录默认是当前目录的 `.nvidia_probe`。选择卸载时会在 Python 进程退出后删除 `.nvidia_probe` 整个程序目录，只保留 `nvidia_probe_results` 中的测试结果。如果检测过程中按 Ctrl+C 中断，会改为询问是否删除程序，默认不删除。若想运行后不询问卸载，可追加参数：
...
如果测试完成后当时选择了保留程序，后续想手动卸载程序并保留测试结果，可在当初运行一键命令的同一目录执行：

```bash
bash -c 'set -e; install_dir="${NVIDIA_PROBE_INSTALL_DIR:-$PWD/.nvidia_probe}"; result_dir="${NVIDIA_PROBE_RESULT_DIR:-$PWD/nvidia_probe_results}"; marker="${NVIDIA_PROBE_CLEANUP_MARKER:-$PWD/.nvidia_probe_cleanup_marker}"; case "$install_dir" in ""|"/"|"$PWD") echo "拒绝删除不安全目录: $install_dir" >&2; exit 1;; esac; if [ -d "$install_dir/results" ]; then dest="$result_dir"; [ -e "$dest" ] && dest="${result_dir}_previous_$(date -u +%Y%m%d_%H%M%S)"; mkdir -p "$(dirname "$dest")"; mv "$install_dir/results" "$dest"; echo "已迁移旧结果目录: $dest"; fi; rm -rf -- "$install_dir" "$marker"; printf "已删除程序目录: %s\n测试结果保留在: %s\n" "$install_dir" "$result_dir"'
```

这个命令只删除默认安装目录 `.nvidia_probe` 和清理标记 `.nvidia_probe_cleanup_marker`，不会删除默认结果目录 `nvidia_probe_results`。如果运行时自定义过 `NVIDIA_PROBE_INSTALL_DIR` 或 `NVIDIA_PROBE_RESULT_DIR`，手动卸载时也要传入同样的环境变量；结果目录应放在安装目录之外。
...
本项目不会在代码里预置 API Key，也不会把 API Key 写入报告或状态文件。Python CLI 运行时按以下优先级读取：
...
远程一键脚本会在安装/更新项目之前先执行同样的 API Key 检查：优先使用 `NVIDIA_API_KEY`，其次使用 `NGC_API_KEY`，如果命令行已传 `--api-key` 则跳过输入，否则会立即隐藏提示输入并导出为 `NVIDIA_API_KEY`，所以后续 Python CLI 不会再次询问。
...
| `--top-free-models` | `20` | 混合 TopN 基础池选择多少个免费候选模型；重要中文模型会在基础池之外额外加入；必须大于 0。 |
...
| `--priority-models` | `deepseek,kimi,glm,minimax` | 逗号分隔的重要中文模型关键词、通配符或完整模型 ID；混合 TopN 会在基础 TopN 之外额外加入匹配模型，留空字符串可关闭。 |
...
| `NVIDIA_API_KEY` | 空 | API Key；一键脚本启动后会优先读取，已设置时不再提示输入。 |
| `NGC_API_KEY` | 空 | API Key 备用环境变量；当 `NVIDIA_API_KEY` 为空时会导出为 `NVIDIA_API_KEY` 使用。 |
...
5. 根据免费规则和类型规则构造候选模型。
6. 默认使用混合 TopN 选择最终候选；其中 `--priority-models` 匹配的重要中文模型会在基础 TopN 之外额外加入，默认覆盖 deepseek、kimi、glm、minimax；如果关闭混合 TopN，则只按 30 天调用量排序。
...
默认 `--top-free-models 20 --stable-top-ratio 0.7 --trending-models 6 --newest-models 0 --new-model-days 14 --priority-models deepseek,kimi,glm,minimax`：

1. 先按 30 天 API 调用量排序，并记录 `usage_rank`。
2. 如果 `--top-free-models` 为空或大于等于候选总数，则检测全部候选。
3. 如果完全没有 30 天调用量数据，则回退为检测全部可确认免费的候选模型。
4. 否则计算基础 TopN 三个池的配额：新模型保底池最多 `--newest-models`；新晋热门池最多 `--trending-models`；剩余名额给稳定热门池，同时尽量满足 `--stable-top-ratio`。默认基础 Top20 会得到 14 个稳定热门池 + 6 个新晋热门池 + 0 个新模型保底。
5. 新晋热门池会先被预留：优先取模型年龄不超过 30 天、且可计算折算 30 天调用量的模型，按折算 30 天调用量排序；这些模型不会再被稳定热门池提前占用。
6. 如果 30 天内可计算热度的新晋候选不足 `--trending-models`，新晋热门池会继续用“较新且有调用量数据”的模型补足，再退回用有调用量排名的候选补足，保证候选总数充足时基础 Top20 仍是 14 个稳定热门池 + 6 个新晋热门池。
...
9. 重要中文模型额外加入：`--priority-models` 中的关键词、通配符或完整 ID 会匹配模型 ID、显示名、provider、owned_by；通过免费/类型过滤且没有出现在基础 TopN 里的匹配模型，会追加到测试列表末尾，默认覆盖 deepseek、kimi、glm、minimax。因此最终测试数可能大于 `--top-free-models`。
10. 传入 `--no-hybrid-topn` 后，如果存在 30 天调用量数据，就只取调用量排名前 `--top-free-models`；如果没有调用量数据，则检测全部候选。
...
检测计划: 获取 77 个可确认 free 模型；类型匹配候选 77 个；本次检测 24 个模型。
已使用混合 TopN 策略选择候选模型：稳定热门=14，新晋热门=6，新模型保底=0，重要中文模型=4（额外加入=4），已选=24。
...
- 如果任务结束时选择了保留程序，后续可使用上文“远程服务器一条命令运行”中的手动一键卸载命令删除 `.nvidia_probe` 和 `.nvidia_probe_cleanup_marker`，同时保留 `nvidia_probe_results`。
...
| 默认测试数量 | 混合 TopN 基础池取免费模型 20 个：14 个稳定热门 + 6 个新晋热门 + 0 个新模型保底；随后额外加入未在基础池中的 deepseek、kimi、glm、minimax 重要中文模型，所以最终测试数可能大于 20 |
...
- `probe_state.json`：断点、运行配置、环境信息、完整原始结果；排名、30 天调用量、日均调用量、筛选原因、原始模型响应等审计信息仍保留在这里。
- `nvidia_models_report.csv`：精简调用决策表，只保留调用模型时真正有价值的字段，默认把可用模型排在前面，再按连接延迟从低到高排序，并保留日均调用量便于判断模型热度。
...
- `api_calls_per_day_display`：日均调用量，优先根据模型上架天数折算；缺少上架时间时按 30 天调用量除以 30 估算，用于直观看模型热度。
...
`selection_rank`、`selection_bucket`、`usage_rank`、`api_calls_30d`、`projected_30d_calls`、`created_at_utc`、`context_length_source`、`max_output_tokens_source` 等审计/来源字段不再写入 CSV/XLSX，避免表格臃肿；CSV/XLSX 只保留可读的 `api_calls_per_day_display` 热度字段。如需排查筛选原因或字段来源，可查看完整的 `probe_state.json`。
...
- 如果能获取 `API calls in the last 30 days` 或 Build 页面中的 `last_month_api_invocation_count`，默认使用混合 TopN 选择基础池前 20 个免费模型，并在报告中输出日均调用量；可用 `--top-free-models` 调整基础池数量；`--priority-models` 匹配的重要中文模型会在基础池之外额外加入。
