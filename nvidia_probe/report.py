...
def calculate_summary(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "failed": failed,
        "skipped": skipped,
        "free_confirmed": sum(1 for item in results if item.get("is_free") is True or str(item.get("is_free", "")).lower() == "true"),
        "free_unknown": sum(1 for item in results if item.get("is_free") in (None, "", "None")),
        ...
        "hybrid_fallback_fill": sum(1 for item in results if item.get("selection_bucket") == "fallback_fill"),
